...
export const useAuthStore = defineStore('auth', () => {
  // 状态 - 过滤无效 token（如 "undefined" 字符串）
  const savedToken = localStorage.getItem('token')
  const token = ref<string>((savedToken && savedToken !== 'undefined' && savedToken.length > 10) ? savedToken : '')
  const user = ref<User | null>(null)

  // 计算属性
  const isLoggedIn = computed(() => !!token.value)
  const isAdmin = computed(() => user.value?.role === 'admin' || user.value?.role === 'superadmin' || user.value?.role === 'test')
  ...
  async function fetchProfile() {
    try {
      ...
      // 仅在确认 token 无效时才清理（401 已由 axios 拦截器处理）
