...
export const pageApi = {
  ...
  async getContentPages() {
    const res = await request.get<any, ApiResult<Array<{ slug: string; title: string }>>>('/pages/content-pages')
    return res.data || res
  },
...
export const pageAdminApi = {
  ...
  async autoFillSection(sectionId: number, replace: boolean = true) {
    const res = await request.post<any, ApiResult<{ filled: number; section_code: string }>>(`/admin/pages/sections/${sectionId}/auto-fill`, { replace })
    return res.data || res
  },
  async autoFillChannel(code: string) {
    const res = await request.post<any, ApiResult<Record<string, number>>>(`/admin/pages/channels/${encodeURIComponent(code)}/auto-fill`)
    return res.data || res
  },
