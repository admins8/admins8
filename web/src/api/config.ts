...
/** 公开配置缓存：避免并发重复请求 */
let publicConfigsCache: Promise<SiteConfigItem[]> | null = null
...
export const configApi = {
  ...
  getPublicConfigs() {
    if (publicConfigsCache) return publicConfigsCache
    publicConfigsCache = request.get<any, SiteConfigItem[]>('/config/public/all')
    publicConfigsCache.catch(() => { publicConfigsCache = null })
    return publicConfigsCache
  },
