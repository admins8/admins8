...
export function buildCollectorPluginRows(rules: CollectorRuleRow[], logs: CollectorLogRow[]): CollectorPluginTableRow[] {
  return rules.map((rule) => {
    ...
    const entryUrls = rule.rule?.entryUrls || [rule.entryUrl]
    const displayUrl = entryUrls.length > 1 ? `${entryUrls[0]} 等${entryUrls.length}个网址` : entryUrls[0]
    ...
    return {
      ...
      entryUrl: displayUrl || rule.entryUrl,
