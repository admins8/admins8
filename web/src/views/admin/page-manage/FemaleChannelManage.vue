  ...
  <div class="female-channel-manage">
    <div class="page-header">
      ...
      <div>
        ...
        <el-button type="success" :loading="autoFillingAll" @click="autoFillAllAction">一键拉取全部</el-button>
        ...
        <div v-for="section in group.sections" :key="section.id" class="section-card">
          <div class="section-head">
            ...
            <div>
              ...
              <el-button size="small" type="warning" :loading="autoFillingSection === section.id" @click="autoFillSectionAction(section)">一键拉取</el-button>
...
const autoFillingSection = ref(0)
...
async function autoFillSectionAction(section: PageChannelSection) {
  try {
    await ElMessageBox.confirm(
      `将从书库中自动拉取书籍填充"${section.title}"区块，会替换现有条目，确认吗？`,
      '一键拉取确认',
  ...
  autoFillingSection.value = section.id
  try {
    const res = await pageAdminApi.autoFillSection(section.id, true)
    await loadChannel()
    ElMessage.success(`已拉取 ${res.filled} 本书籍到"${section.title}"`)
  } catch (e: any) {
    ElMessage.error(e?.message || '拉取失败')
  } finally {
    autoFillingSection.value = 0
  }
}

async function autoFillAllAction() {
  try {
    await ElMessageBox.confirm(
      '将从书库中自动拉取书籍填充女生频道所有推荐区块（幻灯片除外），会替换现有条目，确认吗？',
      '一键拉取全部确认',
      { type: 'warning', confirmButtonText: '确认拉取全部', cancelButtonText: '取消' },
    )
  } catch {
    return
  }
  ...
  try {
    const res = await pageAdminApi.autoFillChannel('female')
    await loadChannel()
    const total = Object.values(res).reduce((a: number, b: any) => a + b, 0)
    const details = Object.entries(res)
      .filter(([, n]) => n > 0)
      .map(([code, n]) => `${code}:${n}本`)
      .join(', ')
    ElMessage.success(`拉取完成，共填充 ${total} 本书籍。${details}`)
  } catch (e: any) {
    ElMessage.error(e?.message || '拉取失败')
  } finally {
