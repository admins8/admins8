<template>
  <div class="reader-page">
    ...
    >
      ...
      <div v-if="bookStore.loading && !isAppendingChapter" class="content-loading">
        <el-icon :size="32" class="loading-icon"><Loading /></el-icon>
        <span>加载中...</span>
      </div>
      <div v-else-if="hasReadableContent" class="content-text">
        <!-- 滚动模式：流式渲染多个章节 -->
        <template v-if="readerSettings.readingMode === 'scroll' && streamContents.length > 0">
          <template v-for="(item, idx) in streamContents" :key="item.url">
            <h2
            ...
            <ad-slot v-if="idx === 0" position="reader_middle" class="reader-ad reader-ad-middle" />
            <div
              class="content-body"
              :style="contentBodyStyle"
              v-html="formatContentHtml(item.content)"
            />
            <div v-if="idx < streamContents.length - 1" class="chapter-divider" />
          </template>
        </template>
        <!-- 分页模式：单章渲染 -->
        <template v-else>
...
import { configApi, bookApi, type AlternateSource, type Chapter } from '@/api'
...
// 章节内容预缓存（加速阅读翻章）
const chapterContentCache = new Map<string, { title: string; content: string; index: number }>()
const preloadIndex = ref(0)

/** 预缓存后续章节内容 */
function preloadNextChapters(startPos: number, count: number = 5) {
  const chapters = bookStore.chapters
  for (let i = 1; i <= count; i++) {
    const idx = startPos + i
    if (idx >= chapters.length) break
    const chapter = chapters[idx]
    if (!chapter?.url || chapterContentCache.has(chapter.url)) continue
    const cacheKey = chapter.url
    bookApi.getBookContent(bookUrl.value, chapter.url, readerBookInfo.value?.sourceUrl)
      .then((res: any) => {
...
const streamContents = ref<StreamChapter[]>([])
...
function formatContentHtml(contentRaw: string): string {
  const content = cleanReaderContent(contentRaw)
  let cursor = 0
...
const formattedContent = computed(() => {
  return formatContentHtml(bookStore.currentChapter?.content || '')
})
...
const hasReadableContent = computed(() => {
  if (readerSettings.value.readingMode === 'scroll' && streamContents.value.length > 0) {
    return streamContents.value.some(item => item.content.trim().length > 0)
  }
...
async function jumpToChapter(chapter: Chapter) {
  if (isCurrentChapter(chapter) && streamContents.value.length === 1 && streamContents.value[0]?.url === chapter.url) {
    showCatalog.value = false
    return
  }
  showCatalog.value = false
  await loadChapterContent(chapter, false)
}

async function prevChapter() {
  if (!hasPrev.value) return
  const chapter = bookStore.chapters[currentChapterPosition.value - 1]
  if (chapter) await loadChapterContent(chapter, false)
}

async function nextChapter(append = false) {
  if (!hasNext.value) return
  const chapter = bookStore.chapters[currentChapterPosition.value + 1]
  if (chapter) await loadChapterContent(chapter, append)
}

/**
 * 自定义平滑滚动（requestAnimationFrame + easeOutCubic）
 * 替代浏览器原生 scroll-behavior: smooth，提供更一致的动画体验
 */
function smoothScrollTo(element: HTMLElement, targetTop: number, duration = 400) {
...
function resetReaderViewport() {
  if (readerContentRef.value) {
    smoothScrollTo(readerContentRef.value, 0, 300)
  }
...
async function goNextPage() {
  ...
  runPageAnimation()
  smoothScrollTo(el, Math.min(el.scrollTop + pageHeight, el.scrollHeight), 350)
}
...
async function goPrevPage() {
  ...
  runPageAnimation()
  smoothScrollTo(el, Math.max(el.scrollTop - pageHeight, 0), 350)
}
...
/**
 * 根据滚动位置检测当前可见的章节，并更新 bookStore.currentChapter
 */
function detectVisibleChapterFromScroll() {
  if (readerSettings.value.readingMode !== 'scroll' || streamContents.value.length <= 1) return
  const el = readerContentRef.value
  if (!el) return
  const titles = el.querySelectorAll<HTMLElement>('.content-title[data-chapter-index]')
  ...
  titles.forEach((title) => {
    const idx = Number(title.dataset.chapterIndex)
    const dist = Math.abs(title.offsetTop - viewportCenter)
    if (dist < closestDist) {
      closestDist = dist
      closestIndex = idx
    }
  ...
  if (closestIndex >= 0 && closestIndex !== bookStore.currentChapter?.index) {
    const streamItem = streamContents.value.find(item => item.index === closestIndex)
    if (streamItem) {
      bookStore.currentChapter = {
        ...(bookStore.currentChapter || {}),
        index: streamItem.index,
...
async function handleReaderScroll(event: Event) {
  ...
  // 流式阅读：检测当前可见章节（无论是否开启自动下一章）
  if (readerSettings.value.readingMode === 'scroll') {
    detectVisibleChapterFromScroll()
  }
...
async function loadChapterContent(chapter: Chapter, append = false) {
  isAppendingChapter.value = append
  try {
    stopSpeech()
    // 优先从缓存读取
    const cached = chapterContentCache.get(chapter.url)
    if (cached) {
      bookStore.currentChapter = cached as any
      chapterContentCache.delete(chapter.url)
    } else {
    ...
    if (!append) {
      ...
      streamContents.value = [{
        index: chapter.index,
        title: chapter.title,
        content: bookStore.currentChapter?.content || '',
        url: chapter.url,
      }]
      resetReaderViewport()
    } else {
      // 追加模式：将新章节追加到流式内容末尾
      const alreadyExists = streamContents.value.some(item => item.url === chapter.url)
      if (!alreadyExists) {
    ...
    // 预缓存后续章节
    const pos = getCurrentChapterPosition(bookStore.chapters, chapter)
    if (pos >= 0) {
      preloadNextChapters(pos, 5)
    }
  } catch {
    // 区分权限错误（如游客章节限制）和真正的内容加载失败
    if (bookStore.contentAccessError?.data?.requiresLogin) {
      // 游客阅读限制等权限问题，不触发自动换源推荐
      return
    }
...
async function openSwitchSource() {
  ...
  const bookInfo = readerBookInfo.value
  doSearchAlternateSources()
}

function doSearchAlternateSources(excludeBookUrls?: string[]) {
  const bookInfo = readerBookInfo.value
  sourceLoading.value = true
  bookStore.streamAlternateSources(
    ...
    (hasMore) => {
      ...
      sourceHasMore.value = hasMore
      if (!excludeBookUrls?.length && alternateSources.value.length === 0) {
...
function handleSourceListScroll(event: Event) {
  ...
  if (el.scrollTop + el.clientHeight >= el.scrollHeight - 24) {
    ...
    // 已显示全部结果且还有更多书源时，触发继续搜索
    if (visibleSourceCount.value >= alternateSources.value.length && !sourceLoading.value && sourceHasMore.value) {
      const excludeBookUrls = alternateSources.value.map(s => s.bookUrl)
      doSearchAlternateSources(excludeBookUrls)
    }
  }
}
...
async function switchToSource(source: AlternateSource) {
  ...
  const loadingMsg = ElMessage.info({ message: '正在切换书源，请稍候...', duration: 0, grouping: true })
  bookStore.stopAlternateSourceSearch()
  try {
