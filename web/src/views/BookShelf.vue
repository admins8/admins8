<template>
  <div class="bookshelf-page">
    ...
    >
          ...
          <el-menu-item index="favorites" class="mobile-hidden-shelf-entry">
    ...
    <el-dialog v-model="showSearch" title="搜索书籍" width="600px" destroy-on-close>
      ...
      <div v-if="bookStore.searchResults.length > 0" class="search-results" ref="searchResultsEl" @scroll="onSearchScroll">
          ...
          <div class="result-info" @click="goToDetail(item)">
            <div class="result-name-row">
              ...
              <span class="exact-match-wrap">
                <el-tag size="small" type="success" class="result-kind-tag">{{ item._matchLabel || '精确匹配' }}</el-tag>
              </span>
        ...
        <div
          ...
          class="search-result-item search-result-item-new related-result"
        >
          <!-- 左侧封面 -->
          <div class="result-cover-wrap">
            <el-image
          ...
          <div class="result-info" @click="goToDetail(item)">
            <div class="result-name-row">
              <span class="result-name">{{ item.name }}</span>
              <el-tag size="small" type="info" class="result-kind-tag">{{ item._matchLabel || '相关结果' }}</el-tag>
              <el-tag v-if="(item as any)._local" size="small" type="warning" class="result-kind-tag">已缓存</el-tag>
              <el-tag v-else-if="(item as any)._cached" size="small" type="info" class="result-kind-tag">缓存中</el-tag>
              <el-tag v-if="item.kind" size="small" type="warning" class="result-kind-tag">{{ item.kind }}</el-tag>
            </div>
...
import {
  Search, Plus, Grid, List, Clock, CircleCheck, Star,
...
const filteredBooks = computed(() => {
  ...
  if (activeCategory.value === 'favorites') {
    ...
    books = books.filter(b =>
      b.durChapterIndex != null &&
      !(b.totalChapterNum != null && b.totalChapterNum > 0 && b.durChapterIndex >= b.totalChapterNum - 1)
    )
      .sort((a, b) => {
        const ta = a.lastReadTime || ''
        const tb = b.lastReadTime || ''
        return tb.localeCompare(ta)
      })
  } else if (activeCategory.value === 'recent') {
    // 最近阅读：有阅读记录的，按最后阅读时间倒序
    books = books.filter(b => b.lastReadTime && b.durChapterIndex !== undefined && b.durChapterIndex !== null)
      .sort((a, b) => {
        const ta = a.lastReadTime || ''
        const tb = b.lastReadTime || ''
        return tb.localeCompare(ta)
    ...
    // 已读完：当前章节 >= 总章数 - 1
    books = books.filter(b =>
      b.totalChapterNum != null &&
      b.totalChapterNum > 0 &&
      b.durChapterIndex != null &&
      b.durChapterIndex >= b.totalChapterNum - 1
    )
  }
  return books
})
...
.search-result-item.search-result-item-new.related-result {
  opacity: 0.92;
}
