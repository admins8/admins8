...
let validateEventSource: { close: () => void } | null = null
...
function startBatchValidate() {
  ...
  // 使用 fetch POST + ReadableStream 替代 EventSource，避免 URL 长度超限
  async function readStream() {
    try {
      const resp = await fetch('/api/sources/validate-stream', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          keyword: kw,
          timeout: validateTimeout.value * 1000,
          concurrency: validateConcurrency.value,
          ids,
        }),
      })
      ...
      finalizeValidate()
      if (validateDone.value < validateTotal.value && validateTotal.value > 0) {
        ElMessage.error('验证连接异常已中断')
      } else if (validateTotal.value > 0) {
  ...
  function parseSSEChunk(chunk: string) {
    ...
    if (eventName === 'start') {
      try {
        ...
        if (typeof parsed?.total === 'number') validateTotal.value = parsed.total
      } catch {}
    } else if (eventName === 'progress') {
      try {
        const parsed = JSON.parse(data)
        validateDone.value++
        if (parsed.ok) validateOkCount.value++
        else validateFailCount.value++

        const row = sources.value.find((s) => s.id === parsed.id)
        if (row) {
          row.lastCheckStatus = parsed.ok ? 1 : 2
          row.lastCheckMessage = parsed.message || ''
          row.respondTime = parsed.respondTime || 0
          row.lastCheckTime = Date.now()
          row._validating = false
        }
        validateLastEvent.value = {
          id: parsed.id,
          name: parsed.name || row?.bookSourceName || '',
          ok: !!parsed.ok,
      ...
      try {
        const parsed = JSON.parse(data)
        ElMessage.error(parsed.message || '验证出错')
      } catch {}
    }
  }
