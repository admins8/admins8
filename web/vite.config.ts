...
function fetchSiteConfig(): Promise<SiteConfigMap> {
  const apiBase = process.env.SITE_CONFIG_API_BASE || process.env.VITE_API_PROXY_TARGET || 'http://localhost:5341'
...
export default defineConfig({
  ...
  server: {
    ...
    proxy: {
      '/api': {
        target: process.env.VITE_API_PROXY_TARGET || 'http://localhost:5341',
        changeOrigin: false,
