...
const friendlyLinkService_1 = require("./friendlyLinkService");
const aliases = {
    网站名: 'siteName',
    年份: 'year',
    书名: 'bookName',
...
function buildFriendlyLinksFallback(links) {
    const items = links
...
async function renderFrontendHtml(indexPath, requestUrl) {
    ...
    const [configs, friendlyLinks] = await Promise.all([
        loadSiteConfigMap(),
        (0, friendlyLinkService_1.getPublicFriendlyLinks)().catch(() => []),
    ]);
