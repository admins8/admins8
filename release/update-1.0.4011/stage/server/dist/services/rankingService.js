...
exports.RANK_TYPES = void 0;
exports.getActiveCategories = getActiveCategories;
exports.getCachedCategories = getCachedCategories;
exports.refreshCategoriesCache = refreshCategoriesCache;
exports.isValidRankType = isValidRankType;
exports.isValidCategory = isValidCategory;
exports.normalizeRankingInput = normalizeRankingInput;
exports.autoDetectCategory = autoDetectCategory;
exports.pickCategory = pickCategory;
exports.buildAutoRankingsFromBooks = buildAutoRankingsFromBooks;
exports.normalizeAuthor = normalizeAuthor;
exports.normalizeBookName = normalizeBookName;
exports.dedupeRankingItems = dedupeRankingItems;
...
let _cachedCategoriesAt = 0;
const CATEGORIES_CACHE_TTL_MS = 30000;
async function getActiveCategories(forceRefresh = false) {
    const now = Date.now();
    if (!forceRefresh && _cachedCategories.length > 0 && (now - _cachedCategoriesAt) < CATEGORIES_CACHE_TTL_MS) {
        return _cachedCategories;
    }
    try {
        const rows = await (0, database_1.query)(`SELECT name FROM book_categories WHERE is_active = 1 ORDER BY sort_order ASC, id ASC`);
        const names = Array.isArray(rows) && rows.length > 0
            ? rows.map((r) => String(r.name).trim()).filter((n) => n.length > 0)
            : [];
        // 始终包含"全部"作为第一个分类
...
function isValidRankType(code) {
    if (!code)
        return false;
    return exports.RANK_TYPES.some((r) => r.code === code);
}
...
function normalizeRankingInput(input) {
    ...
    return {
        ...
        is_active: input.is_active === 0 || input.is_active === false ? 0 : 1,
    };
}
// ============================================================
 ...
 * @param info.intro   简介
 */
function autoDetectCategory(info) {
...
function toRecord(seed, type, idx) {
    return {
        name: seed.name,
        author: seed.author || '',
 ...
 */
function buildAutoRankingsFromBooks(seeds, limitPerRank = 20) {
    ...
    const complete = sortBy(safe.filter((s) => !!s.isComplete), (a, b) => (b.readers || 0) - (a.readers || 0))
        .slice(0, limitPerRank)
