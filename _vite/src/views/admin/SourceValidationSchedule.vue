...
import { sourceApi, type SourceValidationScheduleSettings } from '@/api'
...
async function loadSettings() {
  loading.value = true
  try {
    applySettings(await sourceApi.getValidationSchedule())
  } finally {
    loading.value = false
  }
}
...
async function saveSettings() {
  saving.value = true
  try {
    const settings = await sourceApi.updateValidationSchedule({ ...form })
...
async function runNow() {
  running.value = true
  try {
    const { settings } = await sourceApi.runValidationScheduleNow()
    applySettings(settings)
