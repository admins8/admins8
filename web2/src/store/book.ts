...
export function mergeSearchResultLocalPriority(existing: SearchResult, incoming: SearchResult): SearchResult {
  ...
  if (!existingIsLocal && !incomingIsLocal) return incoming
...
function getSearchResultKey(result: SearchResult): string {
  return `${normalizeSearchKeyText(result.name)}|${normalizeSearchKeyText(result.author)}` || result.bookUrl
}
...
export const useBookStore = defineStore('book', () => {
  ...
  const exactSearchResults = computed(() => searchResults.value.filter((item) => item._matchLevel === 'exact'))
  const relatedSearchResults = computed(() => searchResults.value.filter((item) => item._matchLevel !== 'exact'))
