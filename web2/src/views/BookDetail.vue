      ...
      <section class="hero-card">
        ...
        <aside class="author-card">
          <div class="author-header">
            <b>本书作者</b>
            <el-button type="warning" round size="small">关注</el-button>
          </div>
...
async function loadBookDetail() {
  ...
  await Promise.all([loadSeo(), loadChapters(), loadRankings(), loadSocialStats(), loadComments()])
...
async function loadChapters() {
  ...
  try {
    const list = unwrapResponse<Chapter[]>(await bookApi.getChapterList(book.value.bookUrl, book.value.sourceUrl)) || []
...
async function checkCollectorUpdatePrompt() {
  if (!book.value.bookUrl || checkingCollectorUpdate.value || updatingCollector.value) return
  checkingCollectorUpdate.value = true
  try {
    const result = await bookApi.checkCollectorUpdate(book.value.bookUrl)
    if (!result.canUpdate) return
    await ElMessageBox.confirm(
      `${result.message}，是否静默到后台更新？更新完成后会提示并刷新目录。`,
      '发现章节更新',
      {
        type: 'warning',
        confirmButtonText: '后台更新',
        cancelButtonText: '暂不更新',
      }
    )
    updatingCollector.value = true
    ElMessage.info('已开始后台更新章节，完成后会自动提示')
    bookApi.updateCollectorBook(book.value.bookUrl)
      .then(async (updated) => {
        ElMessage.success(`章节更新完成，已更新到 ${updated.chapterCount} 章`)
        await loadChapters()
      })
      .catch((error: any) => {
        ElMessage.error(error?.message || '章节更新失败')
      })
      .finally(() => {
    ...
    if (error !== 'cancel' && error !== 'close') {
      console.warn('[BookDetail] 检查采集更新失败:', error?.message || error)
    }
  } finally {
