...
export const defaultContentCleanerRules: ContentCleanerRules = {
...
export function normalizeContentCleanerRules(input?: string | ContentCleanerRules | null): ContentCleanerRules {
  if (!input) return { ...defaultContentCleanerRules };

  try {
    const raw = typeof input === 'string' ? JSON.parse(input) : input;
    return {
      removeTags: Array.isArray(raw.removeTags) ? raw.removeTags.map(String).filter(Boolean) : defaultContentCleanerRules.removeTags,
      removeTexts: Array.isArray(raw.removeTexts) ? raw.removeTexts.map(String).filter(Boolean) : [],
      removePatterns: Array.isArray(raw.removePatterns) ? raw.removePatterns.map(String).filter(Boolean) : defaultContentCleanerRules.removePatterns,
      replacements: Array.isArray(raw.replacements)
        ? raw.replacements
          .filter((item: any) => item && item.pattern)
          .map((item: any) => ({
    ...
    return { ...defaultContentCleanerRules };
  }
}

export function cleanContent(content: string, customRules?: string | ContentCleanerRules | null): string {
