...
export const DEFAULT_RANDOM_USER_AGENTS = [
...
export function parseUserAgentList(value: string | undefined | null): string[] {
  return String(value || '')
    .split(/\r?\n/)
    .map(item => item.trim())
    .filter(Boolean);
...
export function pickRandomUserAgent(userAgents: string[] = []): string {
  const list = userAgents.length > 0 ? userAgents : DEFAULT_RANDOM_USER_AGENTS;
  return list[Math.floor(Math.random() * list.length)] || DEFAULT_RANDOM_USER_AGENTS[0];
}

export function parseProxyList(value: string): string[] {
  return String(value || '')
    .split(/\r?\n/)
    .map(item => item.trim())
    .filter(Boolean);
...
export function normalizeProxyUrl(proxy: string): string | undefined {
  ...
  try {
    ...
    if (!parsed.port && !['http', 'https'].includes(protocol)) return undefined;
...
export function pickRandomProxy(proxies: string[] = []): string | undefined {
...
export function buildSearchRequestOptions(settings: SearchRequestOptionSettings): UrlOption {
...
export function parseProxyConfig(proxy: string): AxiosRequestConfig['proxy'] {
...
function buildProxyAxiosConfig(proxy: string): Pick<AxiosRequestConfig, 'proxy' | 'httpAgent' | 'httpsAgent'> | undefined {
...
export async function testSearchProxyConnection(input: ProxyConnectionTestInput): Promise<ProxyConnectionTestResult> {
  ...
  let lastResult: ProxyConnectionTestResult | undefined;
  for (const currentTargetUrl of targetUrls) {
    const start = Date.now();
    try {
      const response = await axios.get(currentTargetUrl, {
        timeout: 8000,
        ...proxyAxiosConfig,
        headers: {
...
export async function testSearchProxyPool(input: ProxyConnectionTestInput): Promise<ProxyPoolTestResult> {
  ...
  const limit = 20;
  const results: ProxyConnectionTestResult[] = [];
...
export function parseLooseObjectLiteral(input: string): any {
  try {
...
export function buildHeaders(sourceHeader: string | null): Record<string, string> {
...
function requestBaseUrl(url: string): string {
  const origin = requestOrigin(url);
  return origin ? `${origin}/` : '';
}

function replaceHeaderPlaceholders(value: string, url: string): string {
  return value
    .replace(/\{\{baseUrl\}\}/g, requestBaseUrl(url))
    .replace(/\{\{origin\}\}/g, requestOrigin(url));
}

export function buildRequestHeaders(url: string, headers: Record<string, string>, optionHeaders: Record<string, string> = {}): Record<string, string> {
...
function withoutRetryOverriddenHeaders(headers: Record<string, string>): Record<string, string> {
...
function buildSimpleCollectorHeaders(preserved: Record<string, string>): Record<string, string> {
  return {
    ...preserved,
...
export function buildRetryHeaderProfiles(url: string, headers: Record<string, string>, optionHeaders: Record<string, string> = {}): Array<{ name: string; headers: Record<string, string> }> {
...
function applyRuntimeRequestOptions(headers: Record<string, string>, option: UrlOption): Record<string, string> {
...
function shouldRetryWithNextHeader(error: any): boolean {
...
export function parseSearchUrl(raw: string): { url: string; option: UrlOption } {
...
export function resolveScriptSearchUrl(rawSearchUrl: string, baseUrl?: string): string {
  const raw = rawSearchUrl.trim();
  if (!raw.startsWith('@js:') && !raw.startsWith('<js>')) return rawSearchUrl;
  const code = raw.replace(/^@js:\s*/, '').replace(/^<js>/, '').replace(/<\/js>$/s, '');
  const base = String(baseUrl || '').replace(/\/$/, '');

  const baseConcat = code.match(/baseUrl\s*\+\s*["']([^"']+)["']/);
  if (baseConcat && base) return `${base}${baseConcat[1]}`;

  const urlAssignment = code.match(/url\s*=\s*["']([^"']+)["']/);
  if (urlAssignment) return urlAssignment[1];
...
export async function httpRequest(url: string, headers: Record<string, string>, option: UrlOption = {}): Promise<string> {
  ...
  for (const profile of requestProfiles) {
    ...
    if (proxyAxiosConfig) {
      ...
      axiosConfig.httpAgent = getAgentForUrl(url);
      axiosConfig.httpsAgent = getAgentForUrl(url);
    }
