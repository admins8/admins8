...
export const USER_RECORD_TYPES: UserRecordType[] = [
  'reading',
  'searches',
  'comments',
  'likes',
...
const RECORD_CONFIGS: Record<UserRecordType, UserRecordConfig> = {
...
export function getUserRecordConfig(type: string): UserRecordConfig {
  const config = RECORD_CONFIGS[type as UserRecordType];
  if (!config) throw new Error('不支持的记录类型');
  return config;
}

export function buildUserRecordPagination(input: any) {
...
function keywordWhere(keyword: any, fields: string[], params: any[]): string {
...
async function listReadingOrFavoriteRecords(type: 'reading' | 'favorites', input: any) {
  ...
  if (type === 'reading') where += ' AND (ub.dur_chapter_index > 0 OR ub.dur_chapter_title <> \'\')';

  const fromSql = `FROM user_books ub
    JOIN users u ON u.id = ub.user_id
    LEFT JOIN books b ON b.book_url = ub.book_url
    ${where}`;
  const totalRow = await queryOne(`SELECT COUNT(*) AS count ${fromSql}`, params);
  const list = await query(
    `SELECT ub.id, ub.user_id, u.username, u.email, ub.book_url, b.name AS book_name, b.author,
...
async function listSearchRecords(input: any) {
  const { page, size, offset } = buildUserRecordPagination(input);
  const params: any[] = [];
  let where = 'WHERE 1=1';
  ...
  const totalRow = await queryOne(`SELECT COUNT(*) AS count ${fromSql}`, params);
  const list = await query(
    `SELECT sr.id, sr.user_id, u.username, u.email, sr.keyword, sr.result_count,
...
async function listCommentRecords(input: any) {
  const { page, size, offset } = buildUserRecordPagination(input);
  const params: any[] = [];
  let where = 'WHERE 1=1';
  ...
  const totalRow = await queryOne(`SELECT COUNT(*) AS count ${fromSql}`, params);
  const list = await query(
    `SELECT c.id, c.user_id, u.username, u.email, c.book_url, b.name AS book_name, b.author,
...
async function listLikeRecords(input: any) {
  const { page, size, offset } = buildUserRecordPagination(input);
  const params: any[] = [];
  let where = 'WHERE 1=1';
  where += keywordWhere(input.keyword, ['u.username', 'u.email'], params);
  ...
  const totalRow = await queryOne(`SELECT COUNT(*) AS count ${fromSql}`, params);
  const list = await query(
    `SELECT l.id, l.user_id, u.username, u.email, l.target_type, l.book_url, l.target_id,
...
async function listCheckinRecords(input: any) {
  const { page, size, offset } = buildUserRecordPagination(input);
  const params: any[] = [];
  let where = 'WHERE 1=1';
  where += keywordWhere(input.keyword, ['u.username', 'u.email'], params);
  ...
  const totalRow = await queryOne(`SELECT COUNT(*) AS count ${fromSql}`, params);
  const list = await query(
    `SELECT ci.id, ci.user_id, u.username, u.email, ci.checkin_date, ci.points, ci.created_at
     ${fromSql}
...
export async function listUserRecords(type: string, input: any) {
  getUserRecordConfig(type);
  if (type === 'reading' || type === 'favorites') return listReadingOrFavoriteRecords(type, input);
  if (type === 'searches') return listSearchRecords(input);
  if (type === 'comments') return listCommentRecords(input);
  if (type === 'likes') return listLikeRecords(input);
  if (type === 'checkins') return listCheckinRecords(input);
...
export async function recordUserSearch(input: {
  ...
  try {
    await execute(
      'INSERT INTO user_search_records (user_id, keyword, result_count, ip_address) VALUES (?, ?, ?, ?)',
      [input.userId || null, keyword, Number(input.resultCount || 0), input.ipAddress || '']
    );
  } catch (err: any) {
