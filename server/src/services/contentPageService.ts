...
export function normalizeContentPagePayload(payload: any) {
  return {
...
export async function getPublicContentPage(slug: string) {
  return queryOne('SELECT * FROM content_pages WHERE slug = ? AND is_active = 1 LIMIT 1', [slug]);
}

export async function updateContentPage(slug: string, payload: any) {
  const data = normalizeContentPagePayload(payload);
  if (!data.title) throw new Error('页面标题不能为空');
  await execute(
    `UPDATE content_pages
     SET title = ?, content = ?, is_active = ?, seo_title = ?, seo_keywords = ?, seo_description = ?, updated_at = NOW()
     WHERE slug = ?`,
    [data.title, data.content, data.is_active, data.seo_title, data.seo_keywords, data.seo_description, slug]
  );
