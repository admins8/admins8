...
function toRelativeTemplate(baseUrl: string, urlTemplate: string): string {
...
function findUrlTemplate(value: unknown): string | null {
  if (!value || typeof value !== 'object') return null;
  const record = value as Record<string, any>;
  if (typeof record.urlTemplate === 'string') return record.urlTemplate;
...
export function extractSearchActionTemplate(baseUrl: string, html: string): string | null {
  const $ = cheerio.load(html);
  for (const element of $('script[type="application/ld+json"]').toArray()) {
    const text = $(element).html()?.trim();
    if (!text || !text.includes('SearchAction')) continue;
    try {
      const data = JSON.parse(text);
      const template = findUrlTemplate(data);
      if (template) return toRelativeTemplate(baseUrl, template);
    } catch {
      const match = text.match(/"urlTemplate"\s*:\s*"([^"]+)"/);
      if (match) return toRelativeTemplate(baseUrl, match[1].replace(/\\\//g, '/'));
    }
  }
...
function normalizeApiTemplate(raw: string): string | null {
  ...
  if (!/(api|search|so|book|novel|rank|chapter)/i.test(value)) return null;
...
export function extractApiCandidatesFromHtml(baseUrl: string, html: string): string[] {
  const $ = cheerio.load(html);
  const candidates = new Set<string>();
  const searchAction = extractSearchActionTemplate(baseUrl, html);
  if (searchAction) candidates.add(searchAction);
...
function findArrayPath(value: any, path = '$'): { path: string; item: any } | null {
...
function pickJsonField(item: Record<string, any>, names: string[]): string {
...
export function inferJsonSearchRule(payload: any): SearchRuleDraft {
  ...
  const item = found.item as Record<string, any>;
...
export function buildSearchCandidates(baseUrl: string, keyword: string, extraTemplates: string[] = []): SearchCandidate[] {
...
export function buildApiSource(input: {
  url: string;
  name: string;
  searchUrl: string;
  ruleSearch: SearchRuleDraft;
...
export function buildFallbackGenerationResult(input: {
  url: string;
  name?: string;
  reason?: string;
...
export function buildHeaderProfiles(url: string, customHeaders?: Record<string, string>): Array<{ name: string; headers: Record<string, string> }> {
  const parsed = new URL(url);
  const referer = url;
...
async function generateApiBookSource(
